...
export const updateOrderStatusController = async (req, res) => {
  try {
    ...
    const wasDeliveredOrReceived = currentOrder.orderStatus === 'delivered' || currentOrder.orderStatus === 'received';
    const isNowDeliveredOrReceived = orderStatus === 'delivered' || orderStatus === 'received';
    ...
    if (isNowDeliveredOrReceived && !wasDeliveredOrReceived) {
      balanceUpdate = order.totalPrice;
      transactionData = {
    ...
    if (balanceUpdate !== 0) {
      ...
      await Shop.findByIdAndUpdate(order.shop._id, updateQuery);
    }
