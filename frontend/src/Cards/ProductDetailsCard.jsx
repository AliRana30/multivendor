...
const ProductDetailsCard = ({ product }) => {
  ...
  try {
    ...
    const response = await api.post('/create-new-conversation', {
      groupTitle: `${product._id}_${user._id}_${seller._id}`, 
      userId: user._id,
      sellerId: seller._id,
    });
    ...
    <div className="w-full flex flex-col lg:flex-row gap-8 max-w-6xl mx-auto">
      <div className="flex-shrink-0 w-full lg:w-1/2">
        ...
        <div className="text-black mt-10 flex flex-col">
          ...
          <div className="mb-2">
            Seller Name: {seller?.name || "Unknown Seller"}
          </div>
          <div>
            {seller?._id && (
              <Link to={`/shop/${seller._id}`}>
                {seller?.avatar?.url && (
                  <img
                    src={`http://localhost:5000/${seller.avatar.url}`}
                    alt="Shop Avatar"
          ...
          <button className="text-white bg-black p-4 rounded-xl" onClick={handleMessage}>Send Message</button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-grow space-y-6">
        ...
        <div>
          <div className="flex items-start justify-between mb-2">
            ...
            <div className="flex gap-2 flex-shrink-0">
              <button 
                className="p-2 rounded-full text-black bg-white-100 hover:bg-gray-200 transition-colors"
        ...
        <div className="bg-blue-50 rounded-lg p-3">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xl font-bold text-blue-600">
              ${((product.discountPrice || product.originalPrice) * quantity).toFixed(2)}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button 
            ...
            disabled={product.stock === 0}
            className={`flex-1 font-semibold py-3 px-6 rounded-xl transition-all duration-300 transform hover:scale-[1.02] hover:shadow-lg ${
              product.stock === 0
                ? 'bg-gray-400 text-white cursor-not-allowed opacity-60'
